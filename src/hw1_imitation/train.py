...
def run_training(config: TrainConfig) -> None:
    ...
    ### TODO: PUT YOUR MAIN TRAINING LOOP HERE ### Update: done

    optim = torch.optim.Adam(model.parameters(), lr=config.lr, weight_decay=config.weight_decay)

    # @torch.compile
    def train_step(model, state, action_chunk):
        optim.zero_grad()
    ...
    for epoch in range(config.num_epochs):
        for batch, (state, action_chunk) in enumerate(loader):
            ...
            loss = train_step(model, state, action_chunk)

            step = epoch*(len(loader.dataset) // config.batch_size) + batch
            ...
            if step % config.eval_interval == 0:
                ...
                print(f'step: {step} E: {epoch} loss: {loss}')
